...
function fadeOut(el, cb) {
    ...
    el.style.opacity = 0;
    setTimeout(() => { el.style.display = 'none'; if(cb) cb(); }, 500);
}
function fadeIn(el) {
    el.style.display = '';
    el.style.opacity = 0;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    methodSel.addEventListener('change', () => {
        const m = methods.find(x => x.value === methodSel.value);
        descBox.textContent = m ? m.desc : '';
    });
    ...
    if (method === 'interp') {
        ...
        chartData = {
            ...
            datasets: [
                ...
                {label: 'Veri', data: xnew.map((xi,i) => x.includes(xi) ? y[x.indexOf(xi)] : null), borderColor: 'white', backgroundColor: 'white', fill: false, pointRadius: 4, showLine: false}
            ]
        };
        ...
        chartData = {
            labels: x,
            datasets: [{label: 'Visualization', data: y, borderColor: 'white', backgroundColor: 'rgba(255,255,255,0.2)', fill: false}]
        };
    } else if (method === 'comp') {
